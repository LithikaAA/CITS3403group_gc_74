...
class Track(db.Model):
    ...
    mode: so.Mapped[str] = so.mapped_column(sa.String(20), default="Major")  # Changed to string since you're storing "Major"/"Minor"
    user_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey("users.id"), nullable=False)  # Added this line

    user: so.Mapped["User"] = so.relationship(foreign_keys=[user_id])  # Added this relationship
    user_tracks: so.Mapped[List["UserTrack"]] = so.relationship(
